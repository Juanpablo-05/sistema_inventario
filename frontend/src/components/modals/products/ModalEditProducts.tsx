...
const CADUCIDAD_CATEGORY_KEYWORDS = ["comida", "belleza"];
...
function ModalEditProducts({ producto, onEdit }: ModalEditProductsProps) {
  ...
  const selectedCategory = useMemo(
    () => activeCategorias.find((categoria) => String(categoria.id) === categoriaId) ?? null,
    [activeCategorias, categoriaId],
  );
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!fecha_agregado_p) return;
    if (requiresCaducidad && !fecha_caducidad_p) return;
    if (Number.isNaN(categoryNumber) || categoryNumber <= 0) return;

    const fechaCaducidadFinal = requiresCaducidad ? fecha_caducidad_p : fecha_agregado_p;
